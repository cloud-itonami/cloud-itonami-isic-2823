(ns metalmachmfg.store-contract-test
  "The Store contract as executable tests. Single MemStore backend --
  see `metalmachmfg.store` ns docstring for why a second (Datomic-
...
(deftest batch-upsert-merges-preserving-untouched-fields
  (let [s (seeded)]
    (store/commit-record! s {:effect :batch/upsert :path ["batch-001"]
                             :value {:product-type :continuous-caster}})
    (is (= :continuous-caster (:product-type (store/batch s "batch-001"))))
...
(deftest shipment-propose-commits-and-advances-sequence-and-batch-quantity
  (let [s (seeded)]
    ...
    (is (= 22.0 (:shipped-units (store/batch s "batch-001")))
        "12.0 seeded + 10.0 committed")))
