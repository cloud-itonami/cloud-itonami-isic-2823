(ns metalmachmfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control fabrication/assembly-line
  equipment directly... does NOT authorize or execute maintenance/
  equipment operations... does NOT self-issue a CE (EU Machinery
  Directive 2006/42/EC) or ANSI B11.19 machinery-safety conformity
  mark') implemented faithfully. The single invariant under test:

    MetalMachAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Metallurgy Machinery Plant
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-type :rolling-mill-stand}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :rolling-mill-stand (:product-type (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest invalid-load-test-tonnes-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10b" {:op :log-production-batch :effect :propose :subject "batch-001"
                                   :patch {:load-test-tonnes 99999999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-load-test-tonnes} (-> (store/ledger db) last :basis)))
    (is (not= 99999999.0 (:load-test-tonnes (store/batch db "batch-001"))) "fabricated load-test-tonnes never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "fab-001" :severity :moderate
                                            :description "roll-stand drive pressure anomaly"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
