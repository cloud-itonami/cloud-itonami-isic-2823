(ns metalmachmfg.store
  "SSoT for the metallurgy-machinery (ISIC 2823) plant-operations
  coordination actor, illustrated with rolling-mill and casting-
  machinery manufacturing (rolling-mill stands, continuous casters,
  die-casting machines, extrusion presses, forging presses, wire-
  drawing machines), behind a `Store` protocol so the backend is a
  ...
  `fluidpowermfg.store`, `cloud-itonami-isic-2819`'s own
  `weighpkgmfg.store`), this build ships a single `MemStore` backend
  ...
  is self-contained (no external metallurgy-machinery-manufacturing
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             product-type/load-test-tonnes/quantity/
    ...
    - `equipment`         -- a fabrication-line/assembly-test-bench
    ...
    - `maintenance`       -- a scheduled fabrication/assembly-test-
                             bench-equipment maintenance-window DRAFT
                             against a piece of equipment
                             (`metalmachmfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound metallurgy-machinery
                             shipment DRAFT (`metalmachmfg.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (metalmachmfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (metalmachmfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-type :rolling-mill-stand
                :model "RM-4Hi-1200" :load-test-tonnes 2200.0
                :quantity-units 40.0 :defect-rate-percent 0.8
                :verified? true :registered? true
                :shipped-units 12.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-type :continuous-caster
                :model "CC-Slab-220" :load-test-tonnes 350.0
                :quantity-units 15.0 :defect-rate-percent 0.5
                :verified? true :registered? true
                :shipped-units 14.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-type :die-casting-machine
                :model "DC-800T" :load-test-tonnes 850.0
                :quantity-units 20.0 :defect-rate-percent 1.1
...
(defn- sample-equipment []
  {"fab-001" {:id "fab-001" :kind :fabrication-line
   ...
   "bench-002" {:id "bench-002" :kind :assembly-test-bench
                :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `metalmachmfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `metalmachmfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  UNVERIFIED/unregistered assembly-test-bench unit (blocks any
