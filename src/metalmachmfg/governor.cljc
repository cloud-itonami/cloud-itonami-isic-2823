(ns metalmachmfg.governor
  "Metallurgy Machinery Plant Operations Governor -- the independent
  compliance layer that earns the MetalMachAdvisor the right to
  ...
  draft-schedule) fabrication/assembly-line equipment, whether a
  proposal secretly tries to self-issue a CE (EU Machinery Directive
  2006/42/EC) or ANSI B11.19 machinery-safety conformity mark (an
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       actuate` or `:assembly-test-
                                       bench/run`) is the 'direct
    ...
    5. Certification-authority
       blocked                     -- ANY proposal (any op) whose own
                                       ...
                                       attempting to self-issue a CE
                                       (EU Machinery Directive
                                       2006/42/EC) or ANSI B11.19
                                       machinery-safety conformity mark
   ...
   10. Invalid product-type        -- for `:log-production-batch`, if
                                       ...
                                       closed known set
                                       (`metalmachmfg.registry/product-
                                       type-valid?`), the batch record
                                       is rejected rather than let a
                                       fabricated product type through.
   11. Invalid load-test-tonnes    -- for `:log-production-batch`, if
                                       the patch declares a
                                       `:load-test-tonnes` that is not
                                       a physically plausible proof-
                                       load test reading
                                       (`metalmachmfg.registry/load-
                                       test-tonnes-valid?`), the batch
   ...
   13. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [metalmachmfg.registry :as registry]
...
(defn- equipment-control-blocked-violations
  ...
  Anything else (direct fabrication/assembly-line-equipment control, a
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :equipment-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") は組立・製造設備の直接操作に該当する可能性があり、恒久的に禁止")}]))
...
(defn- equipment-actuate-blocked-violations
  ...
  to directly actuate fabrication/assembly-line equipment -- this
  ...
  (when (and (= op :schedule-maintenance)
    ...
    [{:rule :equipment-actuate-blocked
      :detail "組立・製造設備の直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
...
(defn- certification-authority-blocked-violations
  ...
  to self-issue a CE (EU Machinery Directive 2006/42/EC) or ANSI
  B11.19 machinery-safety conformity mark -- an authority exclusively
  reserved to the accredited machinery-safety certification body,
  ...
  (let [payload (or (:value proposal) (:patch proposal))]
    (when (true? (:issue-certification? payload))
      [{:rule :certification-authority-blocked
        :detail "機械安全適合性(CE 2006/42/EC・ANSI B11.19等)の自己発行提案は恒久的に禁止 -- 認証機関の専権事項"}])))
...
(defn- invalid-load-test-tonnes-violations
  "For `:log-production-batch`, if the patch declares a
  `:load-test-tonnes` that is not a physically plausible proof-load
  test reading, reject rather than let fabricated/sensor-error data
  through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [tonnes (:load-test-tonnes (:value proposal))]
      (when (and (some? tonnes) (not (registry/load-test-tonnes-valid? tonnes)))
        [{:rule :invalid-load-test-tonnes
          :detail (str tonnes "t は物理的に妥当な耐荷重試験値の範囲外")}]))))
...
(defn check
  "Censors a MetalMachAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (invalid-load-test-tonnes-violations request proposal)
