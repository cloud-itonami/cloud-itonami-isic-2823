(ns metalmachmfg.advisor
  "MetalMachAdvisor -- the *contained intelligence node* for the
  metallurgy-machinery (ISIC 2823) plant-operations coordination
  actor, illustrated here with rolling-mill and casting-machinery
  manufacturing (rolling-mill stands, continuous casters, die-casting
  machines, extrusion presses, forging presses, wire-drawing
  machines).

  It normalizes production-batch patches (product-type/load-test-
  tonnes/quantity/defect-rate), drafts a fabrication/assembly-test-
  ...
  metallurgy-machinery shipment coordination proposal against a
  ...
  a committed record and NEVER a real fabrication/assembly-line-
  equipment actuation, freight dispatch, or machinery-safety
  certification -- see README `What this actor does NOT do`. Every
  output is censored downstream by `metalmachmfg.governor` before
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `metalmachmfg.governor`)
  ...
  `metalmachmfg.governor` HARD-holds any request that doesn't, so a
...
(defn- log-production-batch
  ...
  load-test-tonnes, quantity, defect-rate or verification status. High
...
(defn- schedule-maintenance
  "Draft a fabrication/assembly-test-bench-equipment maintenance-window
  scheduling proposal against a piece of equipment. The advisor
  reports what it can see (equipment verified?/registered?) in its
  rationale, but `metalmachmfg.governor` NEVER trusts this report -- it
...
(defn- flag-safety-concern
  "Draft an equipment-safety/structural-integrity/quality-defect
  ...
  technicality). See `metalmachmfg.phase`: no phase ever adds this op
  to a phase's `:auto` set; `metalmachmfg.governor` also always
...
(defn- coordinate-shipment
  "Draft an outbound metallurgy-machinery shipment coordination
  ...
  `metalmachmfg.governor` NEVER trusts it: it independently recomputes
...
(def ^:private system-prompt
  (str "あなたは冶金機械(圧延機・連続鋳造機・ダイカストマシン・押出プレス・"
       "鍛造プレス・伸線機等の金属加工プラント用重機械)製造工場プラント"
       "運用コーディネーターの助言者です。"
       ...
       "組立・製造等の設備の直接操作(actuate)を絶対に提案してはいけません"
       "(この actor は提案のみを行い、実行は一切行いません)。"
       "機械安全適合性(CE 2006/42/EC・ANSI B11.19等)を自己発行する提案を"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `metalmachmfg.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :metalmachmfg-advisor-proposal
   :op         (:op request)
