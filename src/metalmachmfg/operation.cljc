(ns metalmachmfg.operation
  "MetalMachOperationActor -- one plant-operations coordination request
  = one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (MetalMachAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Metallurgy
...
(defn build
  "Compiles a MetalMachOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `metalmachmfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; MetalMachAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Metallurgy Machinery Plant Operations Governor -- independent censor.
      (g/add-node :govern
